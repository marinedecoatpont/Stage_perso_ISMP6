...
df_unn = '/home/jovyan/private-storage/Decoatpont_m2_ISMP6_personal/2025-07-17/Summary_ULB_fETISh-KoriBU1_2025.csv'
df_lsce_grisli2 = '/home/jovyan/private-storage/Decoatpont_m2_ISMP6_personal/2025-07-17/Summary_ULB_fETISh-KoriBU1_2136.csv'
df_BedMachine = '/home/jovyan/private-storage/Decoatpont_m2_ISMP6_personal/2025-07-17/Summary_ULB_fETISh-KoriBU1_2273.csv'

dfs = [df_lsce_grisli2, df_BedMachine]
...
for df in dfs:
    ...
    target_simu = input("Enter the simulation you want to plot diagnostics for (e.g., ULB_fETISh-KoriBU2): ")
    ...
    grounded_data_target = xr.open_dataset(f'/home/jovyan/private-storage/result/Grounding_mask/{target_simu}/grounding_mask_{target_simu}_{target_exp}.nc')
    grounded_interp_target = ismip.grid_4x4(grounded_data_target.grounding_mask.isel(time=year_index))

    surface_target = ismip.open_file(target_simu, target_exp, 'orog', where)
    surface_interp_target = ismip.grid_4x4(surface_target.orog.isel(time=year_index))

    vx_data_target = ismip.open_file(target_simu, target_exp, 'xvelmean', where)
    ...
    if target_simu in condition:
        ...
        vy_data_target = ismip.open_file(target_simu, target_exp, 'yvelmean', where)
    vy_interp_target = ismip.grid_4x4(vy_data_target.yvelmean.isel(time=year_index))

    grounded_mask_target = ismip.amundsen_mask(grounded_interp_target, where)
    vy_mask_target = ismip.amundsen_mask(vy_interp_target, where)
    vy_mask_target = xr.where(grounded_mask_target == 0, vy_mask_target, np.nan)
    
    vx_mask_target = ismip.amundsen_mask(vx_interp_target, where)
    ...
    surface_mask_target = ismip.amundsen_mask(surface_interp_target, where)
    ...
    for i, row in df.iterrows():
        ...
        print(f'{simu} {exp}')
        output_path = f'/home/jovyan/private-storage/Decoatpont_m2_ISMP6_personal/2025-07-17/Diag/{target_simu}_{comp_simu}_{comp_exp}_{target_year}comparison_diag.png'

        # Si l'image existe déjà, on saute cette itération
        if os.path.exists(output_path):
            print(f"Figure already exists for {target_simu} vs {comp_simu} {comp_exp} ({target_year}), skipping.")
            continue

        grounded_data = xr.open_dataset(f'/home/jovyan/private-storage/result/Grounding_mask/{simu}/grounding_mask_{simu}_{exp}.nc')
        grounded_interp = ismip.grid_4x4(grounded_data.grounding_mask.isel(time = year_index))


        surface = ismip.open_file(simu, exp, 'orog', where)
        surface_interp = ismip.grid_4x4(surface.orog.isel(time = year_index))

        vx_data = ismip.open_file(simu, exp, 'xvelmean', where)
        vx_interp = ismip.grid_4x4(vx_data.xvelmean.isel(time = year_index))

        if simu in condition:
            vy_data = xr.open_dataset(f'/home/jovyan/private-storage/simu/{simu}/{exp}/yvelmean_AIS_{simu}_{exp}.nc', decode_times = False)
        else:
            vy_data = ismip.open_file(simu, exp, 'yvelmean', where)
        vy_interp = ismip.grid_4x4(vy_data.yvelmean.isel(time = year_index))

        grounded_mask_comp = ismip.amundsen_mask(grounded_interp, where)
        
        vy_mask = ismip.amundsen_mask(vy_interp, where)
        vy_mask = xr.where(grounded_mask_comp == 0, vy_mask, np.nan)

        vx_mask = ismip.amundsen_mask(vx_interp, where)
        vx_mask = xr.where(grounded_mask_comp == 0, vx_interp, np.nan)

        surface_mask = ismip.amundsen_mask(surface_interp, where)
        surface_mask_comp = xr.where(grounded_mask_comp == 0, surface_interp, np.nan)
    

        #calcul de la norme
        vitesse = np.sqrt(vx_mask**2 + vy_mask**2)
        vitesse_comp = vitesse * 365.25 * 24 * 3600
        vmin = 1e-1
        vmax = 1e3
        #vmax = 1e4 #UNN_Ua
    ...
    plt.Line2D([0], [0], marker='s', color='w', markerfacecolor='lightblue', markersize=10, label='Ice shelf / Open ocean'),
        ...
        fig.text(0.01, 0.15, "Comparison - Target", va='center', rotation='vertical', fontsize=30, fontweight='bold')
        diff_grounded = grounded_mask_comp - grounded_mask_target
        color_diff = ListedColormap(['brown', 'antiquewhite', 'darkblue'])
    ...
    plt.Line2D([0], [0], marker='s', color='w', markerfacecolor='brown', markersize=10, label='Grounded on comparison'),
        ]
        axes[2, 0].legend(handles=handles_diff, loc='upper right', fontsize=14)

        diff_surface = surface_mask_comp - surface_mask_target
        ...
        diff_vitesse = vitesse_comp - vitesse_target
        ...
        plt.tight_layout()
        fig.savefig(f'/home/jovyan/private-storage/Decoatpont_m2_ISMP6_personal/2025-07-17/Diag/{target_simu}_{comp_simu}_{comp_exp}_{target_year}comparison_diag.png', dpi=300)
